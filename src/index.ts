...
export default function checkForRangeRequest(
    req: Request,
    res?: Response
    ...
    return Promise.resolve().then(() => {
        ...
        if (!contentLength) {
            throw new Error(
                "Response does not have a content length. Cannot use range."
            );
        }
        ...
        function performRead(controller: ReadableStreamController) {
            reader.read().then((readResponse: ReaderReadResult) => {
                ...
                if (currentStart < start && currentEnd <= start) {
                    ...
                    let segment = readResponse.value.slice(
                        0,
                        end - currentStart + 1
                    );
